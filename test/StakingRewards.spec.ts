...
describe('StakingRewards', () => {
  ...
  it('notifyRewardAmount: take half, burn rest', async () => {
    ...
    const burnable = await stakingRewards.totalBurnableTokens()
    expect(burnable).to.be.eq(reward.div(REWARDS_DURATION).mul(REWARDS_DURATION).mul(BURNRATE).div(100))
    ...
    expect(rewardAmount).to.be.eq(reward.div(REWARDS_DURATION).mul(REWARDS_DURATION).mul(100 - BURNRATE).div(100))
  })
  ...
  it('Claimed split should be 0 in Reward cycle', async () => {
    ...
    // fast-forward ~halfway through the reward window
    await mineBlock(provider, rewardEndTime.sub(5).toNumber())
    await stakingRewards.connect(staker).exit()
    let stakerInfo = await stakingRewards.claimedSplits(staker.address)
  ...
  it('Claimed split:: After reward ends', async () => {
    ...
    // fast-forward ~halfway through the reward window
    await mineBlock(provider, rewardEndTime.add(4).toNumber())
    await stakingRewards.connect(staker).exit()
    let stakerInfo = await stakingRewards.claimedSplits(staker.address)
    let hasClaimed = await stakingRewards.hasClaimed(staker.address)
  ...
  it('Claimed split:: After vesting period end', async () => {
    ...
    // console.log("vestingTime ", vestingEndTime.add(rewardEndTime).add(5).toNumber())
    await mineBlock(provider, vestingEndTime.add(rewardEndTime).add(5).toNumber())
    await stakingRewards.connect(staker).exit()
    let stakerInfo = await stakingRewards.claimedSplits(staker.address)
    let hasClaimed = await stakingRewards.hasClaimed(staker.address)
    expect(hasClaimed).to.be.true;
